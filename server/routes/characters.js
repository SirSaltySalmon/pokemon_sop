...
router.get('/character/random', (req, res) => {
    ...
    if (tags) {
        ...
        if (tagList.length > 0) {
            whereClauses.push(`EXISTS (
                SELECT 1 FROM character_tags ct_incl
                JOIN tags t_incl ON ct_incl.tag_id = t_incl.id
                WHERE ct_incl.character_id = characters.id
                AND t_incl.name IN (${tagList.map(() => '?').join(',')})
            )`);
            params.push(...tagList);
        }
    }
    ...
    if (exclude) {
        ...
        if (excludeList.length > 0) {
            whereClauses.push(`NOT EXISTS (
                SELECT 1 FROM character_tags ct_excl
                JOIN tags t_excl ON ct_excl.tag_id = t_excl.id
                WHERE ct_excl.character_id = characters.id
                AND t_excl.name IN (${excludeList.map(() => '?').join(',')})
            )`);
            params.push(...excludeList);
        }
    }

    // Step 1: Get all eligible character IDs
    let sql = 'SELECT characters.id FROM characters ';
    ...
    db.all(sql, params, (err, rows) => {
        ...
        const characterId = randomRow.id;

        // Step 2: Fetch the character by ID
        db.get('SELECT * FROM characters WHERE id = ?', [characterId], (err2, row) => {
            if (err2) {
                console.error(err2);
                return res.status(500).json({ error: 'Database error' });
            }
